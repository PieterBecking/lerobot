...
def merge_stats(stats_list, force_padding=True, target_max_dim=None):
    """
    合并多个数据集的统计信息，可选择保持原始维度或填充
    (Merge statistics from multiple datasets, optionally preserving original dimensions or padding)

    Args:
        stats_list (list): 包含每个数据集统计信息的字典列表
                          (List of dictionaries containing statistics for each dataset)
        force_padding (bool): 是否强制填充到统一维度 (Whether to force padding to unified dimensions)
        target_max_dim (int, optional): 目标最大维度，如果未指定则自动检测 (Target max dimension, auto-detect if not specified)
        ...
        if source_path:
            try:
                ...
                if force_padding:
                    ...
                    print(f"✅ 保持原始维度，跳过填充 (Preserving original dimensions, skipping padding)")
...
def merge_datasets(
    source_folders, output_folder, validate_ts=False, tolerance_s=1e-4, max_dim=None, default_fps=20
):
    """
    ...
    Args:
        ...
        tolerance_s (float): 时间戳不连续性的容差值，以秒为单位 (Tolerance for timestamp discontinuities in seconds)
        max_dim (int, optional): 强制的最大维度。如果为None，自动检测并保持原始维度 (Forced maximum dimension. If None, auto-detect and preserve original dimensions)
    ...
    3. 智能处理维度：相同维度时保持原样，不同维度时填充 (Smart dimension handling: preserve when same, pad when different)
    ...
    # First pass: detect dimensions for each folder
    print("🔍 检测每个数据集的维度... (Detecting dimensions for each dataset...)")
    for folder in source_folders:
        folder_dim = 6  # Default to 6 for robot state
                    ...
                    try:
                        ...
                        if "observation.state" in df.columns:
                            ...
                            if isinstance(first_state, (list, np.ndarray)):
                                ...
                                print(f"✅ 在 {folder} 中检测到 {folder_dim} 维 (Detected {folder_dim} dimensions in {folder})")
                                break
                    except Exception as e:
                        print(f"❌ 检查 {folder} 维度时出错: {e} (Error checking dimensions in {folder}: {e})")
                    break
            if folder_dim != 6:  # Found actual dimensions
                break
        
        folder_dimensions[folder] = folder_dim

    # Auto-detect if we should preserve dimensions or pad
    unique_dimensions = set(folder_dimensions.values())
    print(f"📊 检测到的唯一维度: {unique_dimensions} (Detected unique dimensions: {unique_dimensions})")
    
    if max_dim is None:
        if len(unique_dimensions) == 1:
            # All datasets have the same dimensions - preserve them
            actual_max_dim = list(unique_dimensions)[0]
            print(f"✅ 所有数据集维度相同 ({actual_max_dim})，保持原始维度 (All datasets have same dimensions ({actual_max_dim}), preserving original dimensions)")
            force_padding = False
        else:
            # Mixed dimensions - need to pad to largest
            actual_max_dim = max(unique_dimensions)
            print(f"⚠️  检测到不同维度 {unique_dimensions}，填充到 {actual_max_dim} (Detected different dimensions {unique_dimensions}, padding to {actual_max_dim})")
            force_padding = True
    else:
        # User explicitly specified max_dim
        actual_max_dim = max_dim
        force_padding = True
        print(f"🔧 用户指定维度 {actual_max_dim}，强制填充 (User specified dimension {actual_max_dim}, forcing padding)")
    
    # Update max_dim for the rest of the function
        ...
        try:
            ...
            for episode in episodes:
                ...
                if old_index in stats_map:
                    ...
                    if force_padding and "stats" in stats and folder_dimensions[folder] < max_dim:
                        print(f"🔧 填充统计数据从 {folder_dimensions[folder]} 维到 {max_dim} 维 (Padding stats data from {folder_dimensions[folder]} to {max_dim} dimensions)")
    ...
    if stats_list:
        # Merge global stats
        merged_stats = merge_stats(stats_list, force_padding=force_padding, target_max_dim=max_dim)
        ...
        if all_stats_data:
            ...
            for feature in merged_stats:
                if feature in all_stats_data[0]:
                    ...
                    # Update min/max with conditional padding
                    if "min" in merged_stats[feature] and all("min" in stat for stat in values):
                        ...
                        for val in values:
                            ...
                            if force_padding and len(val_flat) < max_dim:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_dim", type=int, default=None, help="Maximum dimension (default: auto-detect)")
